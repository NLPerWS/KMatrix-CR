...
def parse_args():
    ...
    parser.add_argument(
        "--file_data", type=str, default="[]", help="",
    )
    parser.add_argument(
...
def main(args=None):
    ...
    if args.file_mode.startswith("fin"):
        ...
        fin_path = args.file_mode
    
    try:
        fin_data = json.loads(args.file_data)
    except:
        fin_data = []
    
    if len(fin_data) == 0:
        fin_data = []
        if fin_path.endswith(".jsonl"):
            with open(fin_path, 'r', encoding='utf-8') as f:
                for line in f:
                    proc_line = line.strip()
                    if proc_line:
                        fin_data.append(json.loads(proc_line))
        elif fin_path.endswith(".json"):
            with open(fin_path, 'r', encoding='utf-8') as f:
                fin_data = json.load(f)
        else:
            raise ValueError("file_mode should be either fin|path or path (json or jsonl)")
    ...
    # 使用Distributed特殊方法收集数据
    all_exports = accelerator.gather_for_metrics(export_list)

    # 主进程处理
    if accelerator.is_main_process:
        merged_exports = []
        for proc_exports in all_exports:
            # logger.info(f"------------------proc_exports-----------------------\n{proc_exports}")
            merged_exports.append(proc_exports)
            # if isinstance(proc_exports, list):
            #     merged_exports.extend(proc_exports)
            # elif proc_exports is not None:  # 处理可能的张量
            #     merged_exports.extend(proc_exports)
        
        # 按输入顺序排序
        merged_exports.sort(key=lambda x: x['input_index'])
        
        # 写入文件（原子操作避免部分写入）
        tmp_path = out_json_fn + ".tmp"
        with open(tmp_path, "w") as f:
            for item in merged_exports:
                f.write(json.dumps(item) + "\n")
        os.replace(tmp_path, out_json_fn)  # 原子替换
