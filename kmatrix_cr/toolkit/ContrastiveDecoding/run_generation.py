    ...
    for iidx, prompt_text in enumerate(prompt_lst):
        ...
        if args.do_sample == 'no' and (args.contrastive_decoding == 'student' or args.contrastive_decoding == 'earlystop' or args.contrastive_decoding == 'ngram') :
            ...
            model_kwargs = {
                ...
                "student_min_prob": args.student_min_prob,
                "student_temperature": args.student_temperature,
                "use_cap_student": (args.use_cap_student == 'yes'),
                "use_switch": (args.use_switch == 'yes'),
                "tokenizer": tokenizer,  # 如果模型仍需要 tokenizer
            }
            ...
            output_sequences = model.generate(
                ...
                # 新版关键参数调整
                assistant_model=student_lm,  # 显式指定辅助模型
                pad_token_id=tokenizer.pad_token_id,  # 显式传递
                eos_token_id=tokenizer.eos_token_id,  # 显式传递
                            
                # 其他标准参数
                stopping_criteria=None,      # 若有停止条件需在此指定
...
if __name__ == "__main__":
    ...
    #                             cpus_per_task=10, slurm_constraint='volta32gb')
    # with executor.batch():
