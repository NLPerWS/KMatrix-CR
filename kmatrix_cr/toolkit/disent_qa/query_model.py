...
def query_model(path, checkpoint_name, config_dict, answer_type=""):
    ...
    file_name = save_results_to_file(df, answer_type, checkpoint_name, config_dict, path)
    return file_name
...
def save_results_to_file(df, answer_type, checkpoint_name, config_dict, original_path):
    ...
    file_name = f"{script_directory}/{results_dir}/{model_name}/len{vlen}_{answer_type}_inference.csv"
...
def main(args):
    ...
    model = trained_model
    file_name = query_model(path=args.path,
