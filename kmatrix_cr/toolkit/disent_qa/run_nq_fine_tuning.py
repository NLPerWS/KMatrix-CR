...
"""
# python run_nq_fine_tuning.py --path data/v10-simplified_simplified-nq-train_contextual_baseline_train_split.csv
import os
import argparse
import pandas as pd
import torch
...
if __name__ == '__main__':
    ...
    fine_tune(train_df, val_df, checkpoint_filename, DisentQAModule, model_name,
              ...
              batch_size=config["batch_size"],
              n_epochs=config["n_epochs"],
              checkpoints_dirpath=config["checkpoints_dirpath"],learning_rate=config["learning_rate"])
