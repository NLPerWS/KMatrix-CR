...
import pandas as pd
import os
import kmatrix_cr.toolkit.disent_qa.evaluation_script_squad_v2 as ev
...
def run_eval(path, answer_type, tables=True):
    ...
    if len(df) == 0:
        # 返回一个包含NaN的DataFrame以避免后续错误
        return pd.DataFrame([float('nan')] * len(NAMES), index=NAMES).T
...
def merge_and_save_results(results_dfs, path, tables=True):
    ...
    for i in range(1, len(results_dfs)):
        ...
        m = pd.concat(results_dfs, ignore_index=True)
...
def main(args):
    ...
    for answer_type in ["factual", "counterfactual", "closed_book", "random_context"]:
        res = run_eval(args.path, answer_type, tables=tables)
        if res is not None:
            results_dfs.append(res.reset_index())
    if results_dfs:
        stat_path = merge_and_save_results(results_dfs, args.path, tables=tables)
    else:
        stat_path = ""
