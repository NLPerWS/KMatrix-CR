...
from kmatrix_cr.toolkit.factool.knowledge_qa.pipeline import knowledge_qa_pipeline
from kmatrix_cr.toolkit.factool.code.pipeline import code_pipeline
from kmatrix_cr.toolkit.factool.math.pipeline import math_pipeline
from kmatrix_cr.toolkit.factool.scientific.pipeline import scientific_pipeline
from kmatrix_cr.toolkit.factool.med_doc_qa.pipeline import med_doc_qa_pipeline

class Factool():
    def __init__(self, foundation_model,data_list=[]):
        ...
        self.pipelines = {
                            ...
                            "kbqa_input": knowledge_qa_pipeline(
                                foundation_model = foundation_model, snippet_cnt=10, search_type="input",data_list = self.data_list
                            ),
    ...
    def run(self, inputs):
        ...
        batches = list(filter(lambda x: x != [], batches))
        # print(batches)
        ...
        for batch in batches:
            ...
            if category == 'code':
                ...
                if search_type is None or search_type == "online":
                    ...
                    batch_results = asyncio.run(
                        self.pipelines[category+"_input"].run_with_tool_api_call(
                            [sample['prompt'] for sample in batch],
                            [sample['response'] for sample in batch],
                        )
                    )
