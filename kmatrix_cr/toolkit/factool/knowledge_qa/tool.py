import asyncio
from kmatrix_cr.toolkit.factool.knowledge_qa.google_serper import GoogleSerperAPIWrapper
from kmatrix_cr.toolkit.factool.utils.openai_wrapper import OpenAIEmbed
import json
import os
import numpy as np
import jsonlines
...
class input_search():
    def __init__(self, snippet_cnt,data_list):
        self.snippet_cnt = snippet_cnt
        self.data_list = data_list

    async def run(self, queries):
        snippets_list = []
        for query,cliams in zip(queries,self.data_list):
            text_list = cliams['text'][0:self.snippet_cnt]
            temp_list = [{"content":x,"source":None} for x in text_list]
            snippets_list.append(temp_list)
