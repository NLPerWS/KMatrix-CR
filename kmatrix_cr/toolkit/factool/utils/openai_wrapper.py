...
class OpenAIChat():
    ...
    async def dispatch_openai_requests(
        ...
        # retry>0 do_retry      retry<=0 always retry
        async def _request_with_retry_request(messages, retry=-1):
                ...
                for _ in range(retry):
                    try:
                        ...
                        response = requests.post(api_url, json=param,
                                    headers={'Authorization': f'Bearer {self.api_key}',"Content-Type": "application/json"}, proxies=None)
        ...
        async_responses = [
            _request_with_retry_request(messages)
            for messages in messages_list
        ]
